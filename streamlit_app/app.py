...
if uploaded_file:
    ...
    select_country = st.sidebar.selectbox("select country",['All'] + list(df["country"].dropna().unique()))
    select_product = st.sidebar.selectbox("select product", ["All"] + list(df["item"].dropna().unique()))
    ...
    if select_country != 'All':
        filtered_df = filtered_df[filtered_df["country"] == select_country]
    if select_product != 'All':
        filtered_df = filtered_df[filtered_df["item"] == select_product]
    ...
    avg_last_purchase_days = filtered_df["days_since_last_purchase"].mean()
    avg_tenure = filtered_df["tenure"].mean()

    
    # Display Key Metrics in a table format
    matrix_data = pd.DataFrame({
        "Metric": ["Total Sales", "Total Customers", "Avg Purchase Value", "Churn Rate", "Avg Last purchase days", "Average Tenure"],
        "Value": [f"${total_sales:,.2f}", total_customers, f"${avg_purchase_value:.2f}", f"{churn_rate:.2f}%", f"${avg_last_purchase_days}", f"${avg_tenure}"]
    })
